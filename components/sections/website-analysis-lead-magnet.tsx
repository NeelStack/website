      ...
      <Container size="xl">
        <div className="rounded-3xl border border-border/80 bg-card/80 p-6 sm:p-8 md:p-10 shadow-xl backdrop-blur-md w-full space-y-6 sm:space-y-8 card-hover relative overflow-hidden">
          ...
          <div className="text-center space-y-4 max-w-3xl mx-auto relative z-10">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-violet-500/30 bg-violet-500/10 text-violet-600 dark:text-violet-400 text-xs font-bold uppercase tracking-wider">
            ...
            <h2 className="font-heading text-3xl sm:text-4xl md:text-5xl font-extrabold text-foreground tracking-tight">
              ...
              <span className="text-violet-600 dark:text-violet-400 font-mono">{config.auditPriceFormatted}</span>
            </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
            {AUDIT_POINTS.map((pt) => {
              const Icon = pt.icon
              return (
                <div key={pt.title} className="rounded-2xl border border-border/80 bg-background/60 backdrop-blur-md p-6 space-y-3 transition-all duration-300 hover:border-violet-500/40 hover:bg-background/80 shadow-sm">
